(() => {
  ...
  function toggleTab($trigger) {
    ...
    const $tab = $container.find(" > .tabs__body > .tab").eq($trigger.parent().index());
    $trigger.parent().addClass("current").siblings().removeClass("current");
  ...
  $("pre[data-ajax]").each(function() {
    ...
    fetchJsonString(url).then((codeBlocks) => {
